...
class MultiagentTurtleBot2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self):
        ...
        * /kobuki/laser/scan: Laser Readings
        ...
        rospy.Subscriber("marobot2/kobuki/laser/scan", LaserScan, self._laser_scan_callback_marobot2)
        rospy.Subscriber("marobot3/kobuki/laser/scan", LaserScan, self._laser_scan_callback_marobot3)

        # self._cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)
        self._cmd_vel_pub_marobot1 = rospy.Publisher('marobot1/cmd_vel', Twist, queue_size=1)
        self._cmd_vel_pub_marobot2= rospy.Publisher('marobot2/cmd_vel', Twist, queue_size=1)
        self._cmd_vel_pub_marobot3= rospy.Publisher('marobot3/cmd_vel', Twist, queue_size=1)
    ...
    def _check_odom_ready(self):
        ...
        while self.odom_marobot1 and self.odom_marobot2 and self.odom_marobot3 is None and not rospy.is_shutdown():
            try:
    ...
    def _check_laser_scan_ready(self):
        ...
        while self.laser_scan_marobot1 is None and self.laser_scan_marobot2 is None and self.laser_scan_marobot3 is None and not rospy.is_shutdown():
            try:
                ...
                self.laser_scan_marobot2 = rospy.wait_for_message("marobot2/kobuki/laser/scan", LaserScan, timeout=5.0)
                self.laser_scan_marobot3 = rospy.wait_for_message("marobot3/kobuki/laser/scan", LaserScan, timeout=5.0)
    ...
    def has_crashed(self, min_laser_distance, laser_data):
        """
        It states based on the laser scan if the robot has crashed or not.
        Crashed means that the minimum laser reading is lower than the
        min_laser_distance value given.
