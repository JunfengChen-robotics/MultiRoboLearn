...
class Agent(object):
    ...
    def greedy_actor(self, state):
        if np.random.rand() <= self.epsilon:
            n = random.randrange(self.action_size)
            print("algorithms action are:", n)
            # return random.randrange(self.action_size)
            return n
        else:
            print("algorithms action are:", np.argmax(self.brain.predict_one_sample(state)))
    ...
    def observe(self, sample):
        ...
        if self.memory_model == 'UER':
            ...
            print("Env sample is", str(sample)+"------------>"+str(self.bee_index))
